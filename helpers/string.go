...
// Generate16DigitUniqueID function
func Generate16DigitUniqueID() string {
    rand.Seed(time.Now().UnixNano())
    time.Sleep(time.Duration(rand.Int63n(500) + 250))
    f := fmt.Sprintf("%d", time.Now().UnixNano()/1000)
    return f
}

// PartiallyMatch a string from a slice
func PartiallyMatch(s []string, a string, count int) (string, int) {
    pos := -1
    match := ""
    ...
    check := strings.ToLower(a[0:count])

    for i, str := range s {
        if strings.HasPrefix(strings.ToLower(str), check) {
            match = str
            pos = i
        }
    }
