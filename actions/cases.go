...
func CasesIndex(c buffalo.Context) error {
	...
	columns := []string{
		...
		"over_readings.over_reader_id AS ovrid",
	}
	...
	for _, s := range screenings {
		if s.OverReadingID != nil {
			...
			if s.OverReaderID != nil && *s.OverReaderID == user.ID.String() {
				finishedByMe = append(finishedByMe, s.PID)
			}
			continue
		}
		date := s.CreatedAt
	...
	if len(c.Param("status")) > 0 {
		...
		} else if c.Param("status") == "completed_by_me" {
			status = "111"
			modifier = "="
			inIds = append(inIds, finishedByMe...)
	...
	q = q.PaginateFromParams(c.Value("paginateParam").(url.Values)).Order("created_at DESC")
	...
	c.Set("finishedByMe", len(finishedByMe))
